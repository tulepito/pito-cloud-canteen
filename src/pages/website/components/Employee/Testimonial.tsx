    ...
    <div className="md:mb-32 mb-20">
      <div className="flex flex-col gap-8 md:gap-[60px] max-w-[1024px] mx-auto w-full">
          ...
          {testimonials.map((testimonial, index) => {
            return (
              <SplideSlide key={index} className="group w-full">
                <div className="rounded-2xl flex flex-col justify-between gap-6 h-full">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="text-center">
                      <div className="font-semibold text-base md:text-[28px] mb-1">
                        {testimonial.name}
                      </div>
                  ...
                  <p className="md:flex-1 font-semibold text-2xl md:text-3xl line-clamp-5 md:line-clamp-none mb3">
                    &quot;{testimonial.review}&quot;
                  </p>
        ...
        <div className="flex gap-2 md:gap-5 items-center justify-center">
          <button
            onClick={goPrev}
            className="bg-white border border-solid border-black/10 rounded-lg p-2 md:p-4 hover:bg-black hover:text-white transition-all duration-300 ease-in-out cursor-pointer"
            aria-label="Previous slide">
            <PiCaretLeftLight className="md:text-2xl" />
          </button>
          <button
            onClick={goNext}
            className="bg-white border border-solid border-black/10 rounded-lg p-2 md:p-4 hover:bg-black hover:text-white transition-all duration-300 ease-in-out cursor-pointer"
            aria-label="Next slide">
            <PiCaretRightLight className="md:text-2xl" />
          </button>
        </div>
      </div>
    </div>
