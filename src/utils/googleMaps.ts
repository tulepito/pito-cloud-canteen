 ...
 */
export const getPlaceDetails = (placeId: any, sessionToken: any) =>
  new Promise((resolve, reject) => {
    const { maps } = (window as any).google;

    const serviceStatus = maps.places.PlacesServiceStatus;
    const el = document.createElement('div');
    const service = new maps.places.PlacesService(el);
    const fields = [
      'address_component',
      'formatted_address',
      'geometry',
 ...
 *
 */
export const locationBounds = (latlng: any, distance: any) => {
  const { maps } = (window as any).google;

  const bounds = new maps.Circle({
    center: new maps.LatLng(latlng.lat, latlng.lng),
    radius: distance,
  }).getBounds();
...
export const getLayoutStyles = (
  ...
  const createLatLng = (inst: any, Type: any) => {
    return new Type(inst.lat, inst.lng);
  };

  const createLatLngBounds = (inst: any, Type: any) => {
    return new Type(
      new maps.LatLng(inst.ne.lat, inst.ne.lng),
      new maps.LatLng(inst.sw.lat, inst.sw.lng),
    );
  };
