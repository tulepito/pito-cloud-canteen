...
const radToDegrees = (latlngInRadians: any) => {
...
const obfuscatedCoordinatesImpl = (latlng: any, cacheKey: any) => {
...
const obfuscationKeyGetter = (latlng: any, cacheKey: any) => cacheKey;
 ...
 */
export const obfuscatedCoordinates = (latlng: any, cacheKey = null) => {
  return cacheKey
    ? memoizedObfuscatedCoordinatesImpl(latlng, cacheKey)
    : obfuscatedCoordinatesImpl(latlng, cacheKey);
};
 ...
 */
export const circlePolyline = (latlng: any, radius: any) => {
  const { lat, lng } = latlng;
  const detail = 8;
  const R = 6371;
  const pi = Math.PI;

  const nLat = (lat * pi) / 180;
  const nLng = (lng * pi) / 180;
  ...
  for (let i = 0; i <= 360; i += detail) {
    ...
    let pLat = Math.asin(
      Math.sin(nLat) * Math.cos(d) +
        Math.cos(nLat) * Math.sin(d) * Math.cos(brng),
    );
    const pLng =
      ((nLng +
        Math.atan2(
          Math.sin(brng) * Math.sin(d) * Math.cos(nLat),
          Math.cos(d) - Math.sin(nLat) * Math.sin(pLat),
        )) *
        180) /
      pi;
    pLat = (pLat * 180) / pi;
 ...
 */
export const sdkBoundsToFixedCoordinates = (
  sdkBounds: any,
  fixedPrecision: any,
) => {
  const fixed = (n: any) => Number.parseFloat(n.toFixed(fixedPrecision));
 ...
 */
export const hasSameSDKBounds = (sdkBounds1: any, sdkBounds2: any) => {
  if (
